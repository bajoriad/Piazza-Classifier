...
private:
  ...
  class PairComp
  {
     ...
     public:
      ...
      {
          return copy(pair1.first, pair2.first);
      }
...
public:
  ...
  Value_type& operator[](const Key_type& k)
    {
        Pair_type pair{k, Value_type()};
        if (mapcopy.find(pair) != mapcopy.end())
        {
            return mapcopy.find(pair)->second;
        }
        else
        {
            return mapcopy.insert(pair)->second;
        }
     }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val)
    {
        ...
        if (mapcopy.find(val) != mapcopy.end())
        {
            return {mapcopy.find(val), false};
        }
        else
        {
            return {mapcopy.insert(val), true};
        }
    }
...
private:
  // Add a BinarySearchTree private member HERE.
    BinarySearchTree<Pair_type, PairComp> mapcopy;
};
